...
export class ComponenteComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.componenteService
      ...
      .subscribe({
        next: (res: HttpResponse<IComponente[]>) => {
  ...
  trackId(index: number, item: IComponente): number {
    return item.id!;
  }

  delete(componente: IComponente): void {
    const modalRef = this.modalService.open(ComponenteDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IComponente[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
